...
CMD echo "$HOME/.ops/bin/ops image create --nightly --package node_v14.2.0 -c config.json -i new_img && $HOME/.ops/bin/ops instance create new_img.img -c config.json --instance-name testing_new_img -p 8083 && bash" > commands.txt && cat commands.txt | bash
