...
CMD echo "$HOME/.ops/bin/ops image create --nightly --package node_v14.2.0 -c config.json -i new_img && ops build -c config.json mylilwebserver -i new_img && ls && cd $HOME/.ops/images/ && ls" > commands.txt && cat commands.txt | bash
