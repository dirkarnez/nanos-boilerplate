...
CMD echo "$HOME/.ops/bin/ops image create --nightly --package node_v14.2.0 -c config.json -i new_img && $HOME/.ops/bin/ops build -c config.json -i new_img && $HOME/.ops/bin/ops image list -t onprem" > commands.txt && cat commands.txt | bash
