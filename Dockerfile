...
CMD echo "$HOME/.ops/bin/ops image create --nightly --package node_v14.2.0 -c config.json -i new_img && ls && cd $HOME/.ops/images/ && ls && $HOME/.ops/bin/ops instance create -i new_img testing_new_img -p 8083" > commands.txt && cat commands.txt | bash
